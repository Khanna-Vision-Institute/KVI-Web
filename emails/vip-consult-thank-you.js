...
function buildBodies(rawFirst, bookNote, bookUrl) {
  const firstNameHtml = escapeHtml(rawFirst);
  const text = [
    `Hi ${rawFirst},`,
    '',
  ...
  const html = `
    ...
    <body style="font-family:Georgia,'Times New Roman',serif;line-height:1.65;color:#1a1a2e;max-width:560px;margin:0;padding:24px;">
      ...
      <p style="font-size:15px;">${escapeHtml(bookNote)}</p>
      <p style="margin:28px 0;">
        <a href="${escapeHtml(bookUrl)}" style="display:inline-block;background:linear-gradient(135deg,#0c1a2e 0%,#163256 100%);color:#f0e4c4!important;text-decoration:none;padding:14px 28px;border-radius:10px;font-weight:600;font-size:15px;">Download your PIE book (PDF)</a>
      </p>
