...
const siteHealthPathsRaw = envStr(
  'SITE_HEALTH_PATHS',
  '/,/contact/schedule-consultation/,/contact/forms/,/HeritageFamily,/procedures/laser-vision/smile-laser/,/procedures/laser-vision/lasik/,/procedures/lens-solutions/pie/,/procedures/lens-solutions/evo-icl/,/patients/results/testimonials/,/pricing-financing/special-offers/,/seminar-rsvp.html'
);

const siteHealth = {
  // Automatic cron is OFF by default — use dashboard / POST /api/site-health/run.
  enabled: envBool('SITE_HEALTH_CHECK_ENABLED', false),
  cron: envStr('SITE_HEALTH_CHECK_CRON', '*/5 * * * *'),
  baseUrl: envStr('SITE_HEALTH_BASE_URL', 'https://khannainstitute.com').replace(/\/$/, ''),
  paths: siteHealthPathsRaw
    .split(',')
    .map((p) => p.trim())
    .filter(Boolean),
  timeoutMs: Number(process.env.SITE_HEALTH_TIMEOUT_MS || 12000) || 12000,
  concurrency: Number(process.env.SITE_HEALTH_CONCURRENCY || 4) || 4,
