...
function renderSiteHealthWorkspace() {
  const sh = state.siteHealth;
  if (!sh) {
    return `<section class="card span-all"><div class="card-body">${
      emptyState('Site health loading…', 'Click Run check now to probe key public pages for 404 and 502 errors.')
    }</div></section>`;
  }
  const latest = sh.latest;
  const healthy = latest ? latest.healthy : null;
  const issues = (latest && latest.issues) || [];
  const checks = (latest && latest.checks) || [];
  const history = sh.history || [];

  const statusTone = healthy === true ? 'ok' : healthy === false ? 'bad' : '';
  const statusLabel = healthy === true ? 'Healthy' : healthy === false ? 'Issues found' : 'No run yet';

  return `
    <div class="site-health-kpis">
      <div class="site-health-kpi ${statusTone}">
        <div class="label">Status</div>
        <div class="value">${escapeHtml(statusLabel)}</div>
      </div>
      <div class="site-health-kpi ${(latest && latest.errors404) ? 'bad' : 'ok'}">
        <div class="label">404 errors</div>
        <div class="value">${latest ? latest.errors404 : '—'}</div>
      </div>
      <div class="site-health-kpi ${(latest && latest.errors502) ? 'bad' : 'ok'}">
        <div class="label">502 / 5xx / down</div>
        <div class="value">${latest ? latest.errors502 : '—'}</div>
      </div>
      <div class="site-health-kpi">
        <div class="label">URLs checked</div>
        <div class="value">${latest ? latest.checked : '—'}</div>
      </div>
    </div>

    <div class="site-health-actions">
      <button type="button" class="btn primary" id="runSiteHealthBtn" ${state.siteHealthRunning ? 'disabled' : ''}>
        ${state.siteHealthRunning ? 'Running…' : 'Run check now'}
      </button>
      <span class="site-health-meta">
        Mode: <strong>manual only</strong>
        · Target: <code>${escapeHtml(sh.baseUrl || '')}</code>
        · Auto cron: ${sh.enabled ? 'on' : 'off'}
        ${latest && latest.at ? `· Last run ${escapeHtml(fmtWhen(latest.at))} (${escapeHtml(latest.source || '')})` : ''}
      </span>
    </div>
    ...
    <section class="card span-all">
      <div class="card-head">
        <div>
          ...
          <div class="muted-note">404s and 502/5xx/timeouts on key public pages</div>
        </div>
      </div>
      <div class="card-body">
        ${issues.length ? `<table class="data-table">
          <thead><tr><th>Kind</th><th>Status</th><th>URL</th><th>ms</th></tr></thead>
          <tbody>
            ${issues.map((i) => `<tr>
              <td><span class="issue-kind k-${escapeHtml(i.kind)}">${escapeHtml(i.kind)}</span></td>
              <td>${i.status || '—'}${i.error ? ` <span class="muted-note">(${escapeHtml(i.error)})</span>` : ''}</td>
              <td><a href="${escapeHtml(i.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(i.url)}</a></td>
              <td>${i.ms != null ? i.ms : '—'}</td>
            </tr>`).join('')}
          </tbody>
        </table>` : emptyState('No 404/502 issues', 'All probed pages returned a healthy response on the last run.')}
      </div>
    </section>

    <section class="card span-all">
      <div class="card-head"><h2>Last probe detail</h2><span class="muted-note">${checks.length} URLs</span></div>
      <div class="card-body">
        ${checks.length ? `<table class="data-table">
          <thead><tr><th>Result</th><th>Status</th><th>URL</th><th>ms</th></tr></thead>
          <tbody>
            ${checks.map((c) => `<tr>
              <td><span class="issue-kind k-${escapeHtml(c.kind)}">${escapeHtml(c.ok ? 'ok' : c.kind)}</span></td>
              <td>${c.status || '—'}${c.error ? ` <span class="muted-note">(${escapeHtml(c.error)})</span>` : ''}</td>
              <td><a href="${escapeHtml(c.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(c.url)}</a></td>
              <td>${c.ms != null ? c.ms : '—'}</td>
            </tr>`).join('')}
          </tbody>
        </table>` : '<div class="muted-note">Run a check to see per-URL detail.</div>'}
      </div>
    </section>
    ...
    <section class="card span-all">
      ...
      <div class="card-body">
        ${history.length ? `<table class="data-table">
          <thead><tr><th>When</th><th>Source</th><th>OK</th><th>404</th><th>502+</th><th>Checked</th></tr></thead>
          <tbody>
            ${history.map((h) => `<tr>
              <td>${escapeHtml(fmtWhen(h.at))}</td>
        ...
        </table>` : '<div class="muted-note">No history yet.</div>'}
      </div>
    </section>`;
}
...
function setChannel(channel) {
  if (channel !== 'blog' && channel !== 'seo' && channel !== 'email' && channel !== 'crm' && channel !== 'sitehealth') return;
  closePublishedEditor();
...
function render() {
  ...
  try {
    ...
    else if (state.channel === 'sitehealth') root.innerHTML = renderSiteHealthWorkspace();
...
function wireView() {
  ...
  const runHealthBtn = el('runSiteHealthBtn');
  if (runHealthBtn) {
    runHealthBtn.addEventListener('click', async () => {
      if (state.siteHealthRunning) return;
      state.siteHealthRunning = true;
      render();
      try {
        await api('/site-health/run', { method: 'POST' });
        toast('Site health check finished', 'ok');
        await loadSiteHealth();
        render();
      } catch (err) {
        toast('Site health check failed: ' + err.message, 'bad');
      } finally {
        state.siteHealthRunning = false;
        render();
      }
    });
  }
...
async function loadSiteHealth() {
  try { state.siteHealth = await api('/site-health'); } catch (_) { state.siteHealth = null; }
}

async function refresh() {
  try {
    const [health, stats, approvals, published, analytics, siteHealth] = await Promise.allSettled([
      api('/health'), api('/stats'), api('/approvals'), api('/published-blogs'), api('/analytics/summary'), api('/site-health'),
    ]);
    ...
    if (siteHealth.status === 'fulfilled') state.siteHealth = siteHealth.value;
  } finally {
    state.loaded = true;
    render();
    renderHeaderPills();
...
function init() {
  ...
  window.addEventListener('hashchange', () => {
    ...
    else if (h === 'sitehealth') { state.channel = 'sitehealth'; render(); }
  ...
  else if (initial === 'sitehealth') state.channel = 'sitehealth';
