#!/bin/bash
# Deploy special offers page to production KVI Node server.
set -e
if [[ -z "${SSH_KEY:-}" ]]; then
  for candidate in \
...
echo "Deploying special-offers.html to ${SSH_HOST}..."
scp -i "${SSH_KEY}" -o StrictHostKeyChecking=accept-new \
  "${SCRIPT_DIR}/pricing-financing/special-offers.html" \
  "${SSH_USER}@${SSH_HOST}:${REMOTE_BASE}/pricing-financing/"
...
echo "Done. https://khannainstitute.com/pricing-financing/special-offers/"
