...
router.get('/latest', async (req, res, next) => {
  try {
    console.log('===== /blog/latest route executing =====');
    const blogs = await getAllBlogs();
    console.log('Blogs from database:', blogs.length);
    
    // Add static blogs to the list
    const allBlogs = [...blogs, ...staticBlogs];
    console.log('After adding', staticBlogs.length, 'static blogs:', allBlogs.length);
    ...
    console.log('After removing duplicates:', uniqueBlogs.length);
    ...
    console.log('Final blog count passed to template:', uniqueBlogs.length);
    const testBlog = uniqueBlogs.find(b => b.slug && b.slug.includes('can-lasik-fix'));
    console.log('Test: Found "can-lasik-fix" blog?', testBlog ? 'YES' : 'NO');
    if (testBlog) {
